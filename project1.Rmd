...
## Bonferroni significance tables - 'member'
```{r}
# routes w/ p val lower boferonni at alpha= 0.05
get_sig_table <- function(sig_ind, inverse=F) {
  sig_routes <- routes[sig_ind,]
  ...
  sig_pval <- as.numeric(lapply(sig_fits, function(x) {summary(x)$coefficients[,4]['memberTRUE']}))
  sig_coef <- as.numeric(lapply(sig_fits, function(x) {summary(x)$coefficients[,1]['memberTRUE']}))
  sig_sd <- as.numeric(lapply(sig_fits, function(x) {summary(x)$coefficients[,2]['memberTRUE']}))
  sig_df <- cbind(sig_routes, sig_coef, sig_sd, sig_pval)
  names(sig_df) <- c('station_start', 'station_end', 'Beta_hat', 'std_error', 'pvalue')
  sig_df <- sig_df[order(sig_df$pvalue),]
}
...
# standard fit 
sig_bonferonni_m1 <- get_sig_table(which(route_pvals < 0.05/length(route_pvals)))
print(sig_bonferonni_m1)

# inverse fit
sig_bonferonni_m2 <- get_sig_table(which(route_pvals_inverse < 0.05/length(route_pvals_inverse)), inverse=T)
print(sig_bonferonni_m2)
```
...
```
```{r}
save('pvals_permtest', 'perm_rejects_standard', 'perm_rejects_standard_pval', 'perm_rejects_storey', 'perm_rejects_storey_pval', 'routes_perm_tests_storey', 'routes_perm_tests_storey_sorted', file="data/perm_rejections.RData")
```
